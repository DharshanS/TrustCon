	...
	$AirSegment=$responseArray['SOAPBody']['airAirPriceRsp']['airAirItinerary']['airAirSegment'];
		...
		if($cnnt >0)
		{
			foreach($AirSegment as $mycomm)
			{
			  ...
			  /*echo '<pre>';
			  print_r($mycomm); die;*/
			  if(isset($mycomm['airConnection']) && is_array($mycomm['airConnection']))
			  {
				  $data11[]=  'true';
...
foreach($data['AirPricingSolution']['AirPricingInfo'] as $aps){
	foreach($aps['PassengerType'] as $p){
		if(!isset($pdatadtl[$p['Code']])){
			...
			$pdatadtl[$p['Code']]=array('head'=>1,'currency'=>$currency,'amount'=>number_format($amnt),'tax'=>number_format($tax),'totalamount'=>number_format($amnt),'totaltax'=>number_format($tax),'ptype'=>$ptypefull[$p['Code']]);
...
// july 07 2016
/*$TotalPrice=$data['AirPricingSolution']['TotalPrice'];
$ApproximateTotalPrice=$data['AirPricingSolution']['ApproximateTotalPrice'];
$BasePrice=$data['AirPricingSolution']['BasePrice'];
$ApproximateBasePrice=$data['AirPricingSolution']['ApproximateBasePrice'];
$EquivalentBasePrice=$data['AirPricingSolution']['EquivalentBasePrice'];
$Taxes=$data['AirPricingSolution']['Taxes'];
$ApproximateTaxes=$data['AirPricingSolution']['ApproximateTaxes'];*/
...
$TOTALPRICEVALUE=$TOTALPRICEVALUE - $disAmt;
...
if(isset($myRole) && $myRole == 'agent')
{
	...
	$TOTALPRICEVALUE=$TOTALPRICEVALUE - $webfaireamt;
}
...
<?php } ?>
            ...
            <div class="col-sm-3">
              ...
              <div class="right_panel">
				...
				<?php */ ?>
                  <tr>
                    <td align="left"><span>Total Air Fare :</span></td>
                    ...
                    <div class="well">
                      ...
                      <ul>
                       ...
                       <!-- <li>
                        ...
                        <li>
						   <label><span> </span><br />
                             <a href="images/Passport-01.jpg" target="_blank">Sample passport</a>
						   </label>
                        </li>						
                      </ul>
...
?>
<link href="<?php echo get_template_directory_uri();?>/loader/css.css" rel="stylesheet" type="text/css">
...
jQuery('#step4_mail_details').html('<div style="display: block;" id="please-wait"><div class="order-loading"><div class="order-loading-content"><span class="order-loading-waiting"></span><span class="order-loading-title">Please Wait while we process your booking, this may take a minute</span><span class="order-loading-trivia"></span></div></div></div>');
