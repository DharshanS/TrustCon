...
if(!class_exists('STUser_f'))
{
    class STUser_f extends TravelerObject
    {
        ...
        function dlf_auth( $username, $password ) {
			...
			## Rajib Added
			/*$user_role = ucfirst ($login_status);
            if ( !is_wp_error($user) ) {
				...
				if($user_role != '')
				{
					...
					if ( ! empty( $user_query->results ) )
					{
						...
						{
							if($u->user_login == $username && $user_role == 'Agent')
							{
					...
					$user_query = new WP_User_Query( array( 'role' => 'Agent' ) );
					if ( ! empty( $user_query->results ) )
					{
						foreach ( $user_query->results as $u ) 
						{
							if($u->user_login == $username && $user_role == '')
							{
								$page_login = st()->get_option('page_redirect_to_after_login');
								if(!empty($page_login)){
									$location = esc_url(add_query_arg( 'page_id', $page_login, home_url() ));
        ...
        function get_book_history(){
		 ...
		 function get_book_history_loz($print=0){
            global $current_user;
			global $post;
			global $wpdb;
            ...
            while(have_posts()){
				...
				$TRIPID = get_post_meta(get_the_ID(),'TRIPID',true);
				$PNR = get_post_meta(get_the_ID(),'PNR',true);
				//$provider_code = get_post_meta(get_the_ID(),'provider_code',true);
				...
				$traveltype = get_post_meta(get_the_ID(),'traveltype',true);
				$departdate = get_post_meta(get_the_ID(),'departdate',true);
				$returndate = get_post_meta(get_the_ID(),'returndate',true);
				$bookingdate = get_post_meta(get_the_ID(),'bookingdate',true);
				
				$bookingtime=strtotime($bookingdate);
				//$rows  = $wpdb->get_row( 'SELECT NOW()',ARRAY_N);
				//$now=date("Y-m-d H:i:s",strtotime($rows[0]));
				$now=strtotime("now");
				$cutofftime=24 * 60 * 60; // 24 hour
				
				$transaction_response_code = get_post_meta(get_the_ID(),'TRANSACTION_RESPONSE_CODE',true);
				
				$transaction_id = get_post_meta(get_the_ID(),'TRANSACTION_ID',true);
				
				$post_id=get_the_ID();
				$post_id_hex=md5($post_id);
				
				$tran_msg = '';
				
				switch($transaction_response_code)
				{
					case '0':$tran_msg = 'Approved';
					break;
					case '5':$tran_msg = 'Transaction Rejected';
					break;
					case '1':$tran_msg = 'Transaction with Unknown Error returned';
					break;
				}					
				...
				$booking_reason = get_post_meta(get_the_ID(),'booking_reason',true);
				if($booking_reason=='Pay Later'){
					if(($now - $bookingtime)>$cutofftime){ // disable pay option
						$booking_reason=$booking_reason.'<br><br><a style="color:#aaa;">Pay Now</a>';						
					}else{
					$booking_reason=$booking_reason.'<br><br><a href="'.get_home_url().'/paynow?r='.$post_id_hex.'" target="_blank">Pay Now</a>';
					}
				}
				$html .='
                    <tr>
					    <td class="booking-history-type">'.$TRIPID.'</td>
                        <td class="booking-history-type">'.$PNR.'</td>
						<td class="booking-history-type">'.$ticket_number.'</td>
						<td class="booking-history-type">'.$from.'</td>
						<td class="booking-history-type">'.$to.'</td>
						<td class="booking-history-type">'.date('m/d/y',strtotime($departdate)).'</td>
						<td class="booking-history-type">'.$returndate.'</td>
						<td class="booking-history-type">'.$cost.'</td>
						<td class="booking-history-type">'.date('m/d/y',strtotime($post_date)).'</td>
						<td class="booking-history-type">'.$booking_reason.'</td>
						<td class="booking-history-type">'.$traveltype.' '.$post_author.'</td>
						<td class="booking-history-type">'.$transaction_id.'</td>

						<td class="booking-history-type">'.$tran_msg.'</td>';
						if($print){
							$html .='<td class="booking-history-type"><a style="cursor:pointer;" href="?sc=print-e-ticket-print&i='.$id_item.'" target="_blank">click here</a></td>';
						}
                  $html .='</tr>';
            }
            wp_reset_query();
