...
//echo 'sfddfs'.$connectiondata[0]['SegmentIndex'];
die;*/
## Rajib test
...
## Rajib changed - 1/12/15  ( from CHD to CNN
...
## Rajib Added - 01/12/2015 For Reprofiling
...
/*$soap ='<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
   ...
   <soapenv:Body>
      <air:AirPriceReq TargetBranch="'.$TARGETBRANCH.'" xmlns:air="http://www.travelport.com/schema/air_v29_0" xmlns:com="http://www.travelport.com/schema/common_v29_0">
...
foreach($bookingdata['segmentdtls'] as $b){
	...
	if($b['LinkAvailability']=='') $b['LinkAvailability']= false; // parch why the problem do not know
		//echo '<pre>';
		//print_r($bookingdata['segmentdtls']); print_r($b);die;

$soap .='<air:AirSegment Key="'.$b['Key'].'" Group="'.$b['Group'].'" Carrier="'.$b['Carrier'].'" FlightNumber="'.$b['FlightNumber'].'" Origin="'.$b['Origin'].'" Destination="'.$b['Destination'].'" DepartureTime="'.$b['DepartureTime'].'" ArrivalTime="'.$b['ArrivalTime'].'" FlightTime="'.$b['FlightTime'].'" Distance="'.$b['Distance'].'" ETicketability="'.$b['ETicketability'].'" Equipment="'.$b['Equipment'].'" ChangeOfPlane="'.$b['ChangeOfPlane'].'" ParticipantLevel="'.$b['ParticipantLevel'].'" LinkAvailability="'.$b['LinkAvailability'].'" PolledAvailabilityOption="'.$b['PolledAvailabilityOption'].'" OptionalServicesIndicator="'.$b['OptionalServicesIndicator'].'" ProviderCode="'.$Provider.'">
       <air:AirAvailInfo ProviderCode="'.$Provider.'">
        <!--<air:BookingCodeInfo BookingCounts="J4|C4|D4|I7|Y7|B7|P7|H7|K7|W7|M7|E7|L7|R7|V7|S7|N7|Q7|O7" />-->
		<air:BookingCodeInfo BookingCounts="'.$b['BookingCounts'].'" />
       </air:AirAvailInfo>

<air:FlightDetails Key="'.$b['airFlightDetailsRef'].'" Origin="'.$b['Origin'].'" Destination="'.$b['Destination'].'" DepartureTime="'.$b['DepartureTime'].'" ArrivalTime="'.$b['ArrivalTime'].'" FlightTime="'.$b['FlightTime'].'" TravelTime="'.$b['TravelTime'].'" Equipment="'.$b['Equipment'].'" DestinationTerminal="'.$b['DestinationTerminal'].'" />
         </air:AirSegment>';

}

 $soap .='</air:AirItinerary>
          <air:AirPricingModifiers FaresIndicator="AllFares" CurrencyType="LKR" />';

## Rajib
//echo '<pre>';
//print_r($bookingdata['passengers'][0]); die;
##

 foreach($bookingdata['passengers'][0] as $k=>$psgcode){

 $soap .='<com:SearchPassenger  Code="'.$psgcode.'" Key="'.$k.'" />';
...
/*$soap = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
   <soapenv:Header/>
   <soapenv:Body>
   <air:AirPriceReq TargetBranch="'.$TARGETBRANCH.'" xmlns:air="http://www.travelport.com/schema/air_v34_0" xmlns:com="http://www.travelport.com/schema/common_v34_0" CheckOBFees="true" AuthorizedBy="user">
  <com:BillingPointOfSaleInfo OriginApplication="UAPI" />
  <air:AirItinerary>
    <air:AirSegment Key="zDrQeUM9Roi/ihd1BNbdQw==" FlightNumber="384" Group="0" DepartureTime="2015-12-18T21:50:00" ArrivalTime="2015-12-18T23:50:00" Carrier="9W" Origin="DEL" Destination="BOM" PolledAvailabilityOption="Polled avail used" AvailabilitySource="S" ParticipantLevel="Secure Sell" LinkAvailability="true" ProviderCode="1G">
      <air:Connection />
    </air:AirSegment>
    <air:AirSegment Key="xkFHX1fDSUS2Qd0NUD/yWg==" FlightNumber="377" Group="0" DepartureTime="2015-12-19T06:50:00" ArrivalTime="2015-12-19T08:30:00" Carrier="9W" Origin="BOM" Destination="RPR" PolledAvailabilityOption="Polled avail used" AvailabilitySource="S" ParticipantLevel="Secure Sell" LinkAvailability="true" ProviderCode="1G" />
    <air:AirSegment Key="Vqh9UCU4RROzDmrMIuBuNg==" FlightNumber="7037" Group="1" DepartureTime="2015-12-22T15:10:00" ArrivalTime="2015-12-22T16:55:00" Carrier="9W" Origin="RPR" Destination="BOM" PolledAvailabilityOption="Cached status used. Polled avail exists" AvailabilitySource="P" ParticipantLevel="Secure Sell" LinkAvailability="true" ProviderCode="1G">
      <air:Connection />
    </air:AirSegment>
    <air:AirSegment Key="S2qmsBCPRmmOjT6NIElL1Q==" FlightNumber="361" Group="1" DepartureTime="2015-12-22T21:15:00" ArrivalTime="2015-12-22T23:20:00" Carrier="9W" Origin="BOM" Destination="DEL" PolledAvailabilityOption="Cached status used. Polled avail exists" AvailabilitySource="P" ParticipantLevel="Secure Sell" LinkAvailability="true" ProviderCode="1G" />
  </air:AirItinerary>
  <air:AirPricingModifiers FaresIndicator="PublicAndPrivateFares" />
  <com:SearchPassenger Code="ADT" Key="1" />
  <com:SearchPassenger Code="CNN" DOB="2007-12-03" Age="8" Key="2" />
  <com:SearchPassenger Code="INF" PricePTCOnly="true" DOB="2014-12-03" Age="1" Key="3" />
...
## Rajib Comented - 15/12/2015 For Reprofiling

/*$soap = '';
$soap = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
   <soapenv:Header/>
   <soapenv:Body>
      <air:AirPriceReq  TargetBranch="'.$TARGETBRANCH.'" TraceId="trace" xmlns:air="http://www.travelport.com/schema/air_v32_0" CheckOBFees="true" AuthorizedBy="user">
         <com:BillingPointOfSaleInfo OriginApplication="UAPI" xmlns:com="http://www.travelport.com/schema/common_v32_0"/>
         <air:AirItinerary>';

foreach($bookingdata['segmentdtls'] as $b){

	if($b['LinkAvailability']=='') 
	$b['LinkAvailability']= false;
	else
	$b['LinkAvailability']= true;	 // parch why the problem do not know
		//echo '<pre>';
		//print_r($bookingdata['segmentdtls']); print_r($b);die;
		
  $soap .= '<air:AirSegment Key="'.$b['Key'].'" Group="'.$b['Group'].'" Carrier="'.$b['Carrier'].'" FlightNumber="'.$b['FlightNumber'].'" Origin="'.$b['Origin'].'" Destination="'.$b['Destination'].'" DepartureTime="'.$b['DepartureTime'].'" ArrivalTime="'.$b['ArrivalTime'].'" FlightTime="'.$b['FlightTime'].'"  ETicketability="'.$b['ETicketability'].'" Equipment="'.$b['Equipment'].'" ChangeOfPlane="'.$b['ChangeOfPlane'].'" ParticipantLevel="'.$b['ParticipantLevel'].'" LinkAvailability="'.$b['LinkAvailability'].'" PolledAvailabilityOption="'.$b['PolledAvailabilityOption'].'" OptionalServicesIndicator="'.$b['OptionalServicesIndicator'].'"  AvailabilitySource="LastSeatAvail"  ProviderCode="'.$Provider.'">
 ...
 $soap .= '<com:SearchPassenger  Code="'.$psgcode.'" xmlns:com="http://www.travelport.com/schema/common_v32_0"  Key="'.base64_encode($k+1).'" />';
...
## Rajib Comented Following On 31/12/2015
/*$soap = '';
$soap = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
   <soapenv:Header/>
   <soapenv:Body>
      <air:AirPriceReq TargetBranch="'.$TARGETBRANCH.'" xmlns:air="http://www.travelport.com/schema/air_v32_0" xmlns:com="http://www.travelport.com/schema/common_v32_0" CheckOBFees="true" AuthorizedBy="user">
  <com:BillingPointOfSaleInfo OriginApplication="UAPI" />
  <air:AirItinerary>';
...
foreach($bookingdata['segmentdtls'] as $b){
		...
		$link = "true";
		echo '<pre>';
		print_r($bookingdata['segmentdtls']); die;

  $soap .= '<air:AirSegment Key="'.$b['Key'].'" FlightNumber="'.$b['FlightNumber'].'" Group="'.$b['Group'].'" DepartureTime="'.$b['DepartureTime'].'" ArrivalTime="'.$b['ArrivalTime'].'" Carrier="'.$b['Carrier'].'" Origin="'.$b['Origin'].'" Destination="'.$b['Destination'].'" PolledAvailabilityOption="'.$b['PolledAvailabilityOption'].'" AvailabilitySource="'.$b['AvailabilitySource'].'" ParticipantLevel="'.$b['ParticipantLevel'].'" LinkAvailability="'.$b['LinkAvailability'].'" Equipment="'.$b['Equipment'].'"  ProviderCode="'.$Provider.'">';

## Rajib added
if(!empty($connectiondata))
{  
	//die('ff');
	foreach($connectiondata as $c)
	{
	
		 $soap .= '<air:Connection SegmentIndex="'.$c['SegmentIndex'].'" />';	
...
## Rajib Comented Above On 31/12/2015

## New Revised AirPriceReq - 31/12/2015 - Following
$soap = '';
$soap = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
   <soapenv:Header/>
   <soapenv:Body>
...
## Rajib added For Connection
/*echo '<pre>';
print_r($connectiondata); die('conn');*/

/*if(!empty($connectiondata))
{  
	foreach($connectiondata as $c)
	{
		 $soap .= '<air:Connection SegmentIndex="'.($c['SegmentIndex']+1).'" />';
		 ...
		 $soap .= '<air:Connection SegmentIndex="'.($c['SegmentIndex']+1).'" />';
	 //$soap .= '<air:Connection />';
	 //$soap .= '<air:Connection StopOver="true" SegmentIndex="3"/>';
}*/
...
## New Revised AirPriceReq - 31/12/2015
//echo $soap; die('ttttt');
...
# Rajib Test
	...
	//print_r($responseArray);
	    ...
	    ## Rajib - Start - Error Trapping  ( Know Error Issues
		//echo '<pre>';
		//$data['faultstring'] = $responseArray['SOAPBody']['SOAPFault']['faultstring']; 
	...
	/*echo '<pre>';
	print_r($AirSegment);
	die('AirPriceRsp Array');*/
	# Rajib test

		# Rajib added
		/*echo '<pre>';
		print_r($AirSegment);
		echo $cnnt = count($AirSegment);
		die('fg');*/
		/*print_r($AirSegment[0]['airConnection']);
		die('yuuy');*/
	...
	if(isset($AirSegment['@attributes']['Key'])){
		...
		# Rajib test
		/*if(isset($AirSegment['airConnection'])){
			$connectionData = "'Connection' => true"; 
		}else{
			$connectionData = "'Connection' => false"; 
		}*/
		# Rajib test
                      ...
                      <ul>
                        ...
                        <li>
                          ...
                          <label><span>Expiration date</span><br />
                            ...
                            <input name="passport_exp_date[]" type="text" class="txt_box date-pick" data-date-format="yyyy-mm-dd" data-date-start-date="0d" placeholder="Expiration date" />
              ...
              <!-- Rajib Added - Webbfaire Discount for Agent -->
                ...
                <div class="col-sm-6">
                  ...
                  <div class="payment_options">
                    ...
                    <form name="frmpay" action="<?php echo site_url(); ?>/pay/vpc_php_serverhost_me.php" method="post">
